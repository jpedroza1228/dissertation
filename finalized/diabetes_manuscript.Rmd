...
stand_spatial_coef <- function(object){
...
county <- read_csv('analytic_data/analytic_data2021.csv') %>% 
  janitor::clean_names()
...
census_var <- c('P2_002N', #hispanic/latino, not hispanic/latino by race
                'P4_002N', #hispanic/latino, not hispanic/latino by race for 18+
                'P2_003N', #not hispanic/latino, not hispanic/latino by race
                'P4_003N', #not hispanic/latino, not hispanic/latino by race for 18+
                'P2_006N', #black
                'P4_006N', #black, not hispanic/latino by race for 18+
                'P2_005N', #white
                'P4_005N', #white, not hispanic/latino by race for 18+
                'P2_008N', #asian
                'P4_008N') #asian, not hispanic/latino by race for 18+
...
```{r}
inspect_cor(ca_data, with_col = 'latino') %>% 
  filter(corr > .70) %>% 
  show_plot()
# no asian
# no black
...
mixed_model <- lme4::lmer(inactivity ~ access + violent_crime + median_income_thousand +
                            rural + air_pollution + pop_001 + latino + 
                            (1 | state),
                          data = ca_data,
                          REML = FALSE)
summary(mixed_model)
...
```{r main model, eval = TRUE, echo = FALSE}
set.seed(123021)
ols_dia_envir <- lm(dia_envir,
              data = spatial_join)
summary(ols_dia_envir)

lm.morantest(ols_dia_envir,
             poly_listw,
             zero.policy = TRUE,
             alternative = 'two.sided')

lm.LMtests(ols_dia_envir,
           poly_listw,
           test = c("LMerr", "LMlag", "RLMerr", "RLMlag"),
           zero.policy = TRUE)

sem_dia_envir <- errorsarlm(dia_envir,
                      data = spatial_join,
                      listw = poly_listw,
                      zero.policy = TRUE)
summary(sem_dia_envir,
        Nagalkerke = TRUE,
        zstats = TRUE)
stand_spatial_coef(sem_dia_envir)


ggplot(data = poly_join,
       aes(sem_dia_envir$residuals, sem_dia_envir$fitted.values)) + 
  geom_point() +
  geom_smooth(method = 'lm', se = FALSE)

set.seed(123021)
twotail_p(object = sem_dia_envir,
          listw = poly_listw,
          zero.policy = TRUE,
          nsim = 9999)
```

```{r interactive model, eval = TRUE, echo = FALSE}
ggplot(data = poly_join,
       aes(food_insecure, diabetes)) + 
  geom_point(aes(color = cut(rural, breaks = 3)), alpha = .3) + 
  geom_smooth(method = 'lm', se = FALSE, aes(color = cut(rural, breaks = 3)))

set.seed(123021)
ols_dia_envir_int <- lm(dia_envir_int,
                    data = spatial_join)
summary(ols_dia_envir_int)
interactions::johnson_neyman(ols_dia_envir_int,
                             pred = food_insecure,
                             modx = rural,
                             alpha = .05)

lm.morantest(ols_dia_envir_int,
             poly_listw,
             zero.policy = TRUE,
             alternative = 'two.sided')

lm.LMtests(ols_dia_envir_int,
           poly_listw,
           test = c("LMerr", "LMlag", "RLMerr", "RLMlag"),
           zero.policy = TRUE)

sem_dia_envir_int <- errorsarlm(dia_envir_int,
                            data = spatial_join,
                            listw = poly_listw,
                            zero.policy = TRUE)
summary(sem_dia_envir_int,
        Nagalkerke = TRUE,
        zstats = TRUE)
stand_spatial_coef(sem_dia_envir_int)

spatial_johnson_neyman(model = sem_dia_envir_int,
                       x = "food_insecure",
                       m = "rural",
                       alpha = .05)

theta_plot_sem(model = sem_dia_envir_int,
           x = "food_insecure",
           m = "rural",
           alpha = .05,
           jn = TRUE)
...
ggplot(data = poly_join,
       aes(sem_dia_envir_int$residuals, sem_dia_envir_int$fitted.values)) + 
  geom_point()

set.seed(123021)
twotail_p(object = sem_dia_envir_int,
          listw = poly_listw,
          zero.policy = TRUE,
          nsim = 9999)
```
